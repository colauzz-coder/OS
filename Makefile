...
ASM_FLAGS := -f elf32
QEMU_FLAGS := -m 128M -drive format=raw,file=$(BUILD)/SO.img -no-reboot
...
KERNEL_ASM_OBJ := $(patsubst %.asm, $(BUILD)/%.o, $(KERNEL_ASM_SRC))
...
$(BUILD)/kernel.bin: $(KERNEL_C_OBJ) $(KERNEL_ASM_OBJ)
	mkdir -p $(dir $@)
	$(LD) $(LD_FLAGS) $^ -o $@

$(BUILD)/SO.img: $(BUILD)/kernel.bin $(BUILD)/bootloader.bin
	mkdir -p $(dir $@)
	cat $^ > $@
